...
export default {
  ...
  methods: {
    ...
    async querySearch (queryString, cb) {
      this.result = cb
      this.restaurants = []
      const data = await this.$store.dispatch('searchGetStudent', {
        keyword: this.input
      })
      if (data.length !== 0) {
        data.map(item => {
    ...
    async getkeywordList () {
      ...
      this.grade = ''
      this.classa = ''
      const data = this.tableData = await this.$store.dispatch('getSchoolDeskList', {
        njid: this.grade,
        bjid: this.classa,
        keyword: this.restaurants && this.restaurants.length !== 0 ? this.keyword : '',
