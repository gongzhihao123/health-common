<template>
  <div class='health-check-checkManage'>
    <header>
      <div>
        <span>
          学年：
      ...
      <div>
        ...
        <el-select class="select-width-b" v-model="schoolValue" clearable filterable placeholder="请选择" @change="schoolChange">
          <el-option
      ...
      <div>
        ...
        <el-select class="select-width-s" v-model="gradeValue" @change="gradeChange" clearable filterable placeholder="请选择">
          <el-option
      ...
      <div>
        ...
        <el-select class="select-width-s" v-model="classValue" @change="classChange" filterable clearable placeholder="请选择">
          <el-option
      ...
      <div>
        ...
        <el-select class="select-width-s" v-model="temporaryValue" clearable placeholder="请选择">
          <el-option
      ...
      <div>
        ...
        <el-button size="small" type="primary" :disabled="!schoolValue || !gradeValue || !classValue" @click="batchDownload">批量下载体检反馈</el-button>
      </div>
    </header>

    <main>
      <div>
        <!-- <el-input filterable v-model="keyword"  placeholder="请输入教育ID或学生姓名搜索" :maxlength="20"></el-input> -->
        <el-autocomplete
          v-model="keyword"
        ...
        ></el-autocomplete>
        <el-button @click="sousuo" size="small" type="success" icon="el-icon-search">搜索</el-button>
      </div>
      ...
      <template>
        <el-table
          ...
          <el-table-column
            ...
            label="状态">
            <template slot-scope="scope">
              {{ scope.row.examinationId ? '已签到' : '未签到' }}
            </template>
          </el-table-column>
          <el-table-column
            label-class-name="head"
            ...
            label="操作">
            <template slot-scope="scope">
              <el-button type="success" size="small" v-if="scope.row.examinationId" @click="edit(scope.row)">编辑</el-button>
              <el-button type="primary" size="small" v-else @click="generate(scope.row)">新建体检表</el-button>
...
export default {
  ...
  methods: {
    ...
    async handleSelect (item) {
      ...
      // let data = await this.$store.dispatch('examInfoList', {
      //   xn: this.xnValue,
      //   xxdm: '',
      //   njid: '',
      //   bjid: '',
    ...
    batchDownload () {
      ...
      window.location.href = `${path}/examInfo/exportFeedback?xn=${this.xnValue}&xxdm=${sessionStorage.HEALTH_CHECKMANAGE_SCHOOL}&njid=${sessionStorage.HEALTH_CHECKMANAGE_GRADE}&bjid=${sessionStorage.HEALTH_CHECKMANAGE_CLASS}&lsFlag=${this.temporaryValue}`
    },
    // 下载确认按钮
    downConfirm () {
    ...
    // 下载体检反馈按钮
    async down (val) {
      let data = await this.$store.dispatch('getfeedbackOne', {
    ...
    // 编辑按钮
    edit (val) {
      sessionStorage.HEALTH_BEFORE_PATH = '/healthCheck/checkManage'
    ...
    async schoolChange (val) {
      sessionStorage.HEALTH_CHECKMANAGE_SCHOOL = val
      this.gradeValue = ''
      sessionStorage.HEALTH_CHECKMANAGE_GRADE = ''
      this.classValue = ''
      sessionStorage.HEALTH_CHECKMANAGE_CLASS = ''
      if (sessionStorage.HEALTH_CHECKMANAGE_SCHOOL) {
    ...
    async getGradeList () {
      this.gradeList = await this.$store.dispatch('gradeList', {
        xxdm: sessionStorage.HEALTH_CHECKMANAGE_SCHOOL
      })
    },
    classChange () {
      sessionStorage.HEALTH_CHECKMANAGE_CLASS = this.classValue
    },
    gradeChange () {
      sessionStorage.HEALTH_CHECKMANAGE_CLASS = ''
      this.getClassList()
    ...
    // 获取班级列表
    async getClassList () {
      sessionStorage.HEALTH_CHECKMANAGE_GRADE = this.gradeValue
      this.classList = await this.$store.dispatch('classList', {
        njid: sessionStorage.HEALTH_CHECKMANAGE_GRADE,
        xxdm: sessionStorage.HEALTH_CHECKMANAGE_SCHOOL
      })
      this.classValue = ''
    },
    ...
    async sousuo () {
      ...
      let data = await this.$store.dispatch('examInfoList', {
        ...
        bjid: '',
        lsFlag: '',
        studentId: this.keyword ? this.studentId : '',
        pageNo: this.pageNo,
        pageSize: 10
      })
      this.studentId = ''
    ...
    async getTableData () {
      let data = await this.$store.dispatch('examInfoList', {
        ...
        xxdm: sessionStorage.HEALTH_CHECKMANAGE_SCHOOL,
        njid: sessionStorage.HEALTH_CHECKMANAGE_GRADE,
        bjid: sessionStorage.HEALTH_CHECKMANAGE_CLASS,
    ...
    // 生成体检表
    async generate (val) {
      let data = await this.$store.dispatch('examination', {
        studentId: val.studentId
      })
      if (data === 1) {
        sessionStorage.HEALTH_BEFORE_PATH = '/healthCheck/checkManage'
  ...
  async created () {
    if (sessionStorage.HEALTH_CHECKMANAGE_SCHOOL) {
      this.schoolValue = sessionStorage.HEALTH_CHECKMANAGE_SCHOOL
      await this.getGradeList()
    }
    if (sessionStorage.HEALTH_CHECKMANAGE_GRADE) {
      this.gradeValue = sessionStorage.HEALTH_CHECKMANAGE_GRADE
      await this.getClassList()
    } else {
      this.gradeValue = ''
    }
    if (sessionStorage.HEALTH_CHECKMANAGE_CLASS) {
      this.classValue = sessionStorage.HEALTH_CHECKMANAGE_CLASS
    }
